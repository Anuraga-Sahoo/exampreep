    ...
    return (
        <div className="container mx-auto p-6 max-w-4xl">
            <div className="mb-8 text-center">
                <h1 className="text-3xl font-bold mb-2 text-gray-900">Test Result</h1>
            ...
            <div className="space-y-6">
                {questions.map((q, idx) => {
                    ...
                    if (!isAttempted) statusColor = "border-gray-200 opacity-70";
                    else if (isCorrect) statusColor = "border-green-200 bg-green-50/30";
                    else statusColor = "border-red-200 bg-red-50/30";

                    return (
                        <div key={idx} className={`bg-white p-6 rounded-xl border ${statusColor}`}>
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <span className="inline-block bg-gray-100 text-xs px-2 py-1 rounded mb-2 text-gray-600">
                                        Question {idx + 1} {q.sectionName ? `(${q.sectionName})` : ''}
                                    </span>
                                    <h3 className="text-lg font-medium text-gray-900">{q.text}</h3>
                                    {q.imageUrl && (
                                        <div className="relative w-full max-w-sm h-40 mt-3 rounded border border-gray-200 overflow-hidden">
                            ...
                            <div className="space-y-2 mt-4">
                                {q.options && q.options.map((opt, optIdx) => {
                                    ...
                                    let optClass = "border-gray-200 bg-transparent";
                                    if (isSelected && isThisCorrect) optClass = "border-green-500 bg-green-50";
                                    else if (isSelected && !isThisCorrect) optClass = "border-red-500 bg-red-50";
                                    else if (isThisCorrect) optClass = "border-green-500 bg-green-50 border-dashed"; // Show correct answer if missed
                                    ...
                                    return (
                                        <div key={optIdx} className={`p-3 rounded border flex items-center gap-3 ${optClass}`}>
                                            <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs ${isThisCorrect ? 'bg-green-500 text-white border-green-600' :
                                                (isSelected ? 'bg-red-500 text-white border-red-600' : 'bg-gray-100 text-gray-500')
                                                }`}>
